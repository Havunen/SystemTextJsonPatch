...
public class DynamicObjectAdapter : IAdapter
{
    ...
    {
        if (!TrySetDynamicObjectProperty(target, options, segment, value, out errorMessage))
        {
            return false;
        }
    ...
    public virtual bool TryGet(
        ...
        out string errorMessage)
    {
        if (!TryGetDynamicObjectProperty(target, options, segment, out value, out errorMessage))
        {
            value = null;
            return false;
        }
    ...
    public virtual bool TryRemove(
        ...
        out string errorMessage)
    {
        if (!TryGetDynamicObjectProperty(target, options, segment, out var property, out errorMessage))
        {
            return false;
        }
        ...
        out string errorMessage)
    {
        ...
        if (!TryConvertValue(value, property.GetType(), options, out var convertedValue))
        {
            errorMessage = Resources.FormatInvalidValueForProperty(value);
            return false;
        }

        if (!TryRemove(target, segment, options, out errorMessage))
        {
            return false;
        }

        if (!TrySetDynamicObjectProperty(target, options, segment, convertedValue, out errorMessage))
        {
            return false;
        }
        ...
        out string errorMessage)
    {
        ...
        if (!string.Equals(JsonSerializer.Serialize(property), JsonSerializer.Serialize(convertedValue), StringComparison.Ordinal))
        {
